    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        nav.scrolled { background: rgba(10,10,10,0.85); backdrop-filter: blur(16px); border-bottom: 1px solid var(--border); padding: 14px 48px; }
        .nav-logo { font-family: var(--font-display); font-size: 1.4rem; font-weight: 800; color: var(--text); text-decoration: none; letter-spacing: -0.5px; }
        ...
        .btn-nav { background: var(--lime); color: #000; border: none; padding: 10px 22px; border-radius: 100px; font-family: var(--font-body); font-size: 0.875rem; font-weight: 500; cursor: pointer; transition: all 0.2s; text-decoration: none; display: inline-block; }
        .btn-nav:hover { background: var(--lime-dark); transform: translateY(-1px); }

        /* HERO */
        .hero { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 120px 24px 80px; position: relative; overflow: hidden; }
        .hero::before { content: ''; position: absolute; top: -200px; left: 50%; transform: translateX(-50%); width: 800px; height: 800px; background: radial-gradient(ellipse at center, rgba(200,241,53,0.08) 0%, transparent 70%); pointer-events: none; }
        .hero-badge { display: inline-flex; align-items: center; gap: 8px; background: rgba(200,241,53,0.08); border: 1px solid rgba(200,241,53,0.2); border-radius: 100px; padding: 6px 16px; font-size: 0.8rem; color: var(--lime); margin-bottom: 32px; animation: fadeUp 0.6s ease both; }
        .badge-dot { width: 6px; height: 6px; background: var(--lime); border-radius: 50%; animation: pulse 2s infinite; }
        @keyframes pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.5; transform: scale(0.8); } }
        .hero h1 { font-family: var(--font-display); font-size: clamp(3rem, 8vw, 7rem); font-weight: 800; line-height: 1; letter-spacing: -3px; max-width: 900px; animation: fadeUp 0.6s 0.1s ease both; }
        .hero h1 em { font-style: normal; color: var(--lime); }
        .hero p { max-width: 520px; font-size: 1.125rem; color: var(--muted); margin-top: 24px; font-weight: 300; animation: fadeUp 0.6s 0.2s ease both; }
        .hero-cta { display: flex; gap: 16px; align-items: center; margin-top: 40px; animation: fadeUp 0.6s 0.3s ease both; }
        .btn-primary { background: var(--lime); color: #000; border: none; padding: 16px 36px; border-radius: 100px; font-family: var(--font-body); font-size: 1rem; font-weight: 500; cursor: pointer; transition: all 0.2s; text-decoration: none; display: inline-block; }
        .btn-primary:hover { background: var(--lime-dark); transform: translateY(-2px); box-shadow: 0 8px 32px rgba(200,241,53,0.25); }
        .btn-ghost { background: transparent; color: var(--muted); border: 1px solid var(--border); padding: 16px 36px; border-radius: 100px; font-family: var(--font-body); font-size: 1rem; font-weight: 400; cursor: pointer; transition: all 0.2s; text-decoration: none; display: inline-block; }
        .btn-ghost:hover { color: var(--text); border-color: #444; }
        .hero-stats { display: flex; gap: 48px; margin-top: 72px; animation: fadeUp 0.6s 0.4s ease both; }
        .stat { text-align: center; }
        .stat-num { font-family: var(--font-display); font-size: 2rem; font-weight: 800; color: var(--lime); }
        .stat-label { font-size: 0.8rem; color: var(--muted); margin-top: 4px; }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(24px); } to { opacity: 1; transform: translateY(0); } }

        /* SECTIONS */
        section { padding: 100px 24px; max-width: 1100px; margin: 0 auto; }
        .section-label { font-size: 0.75rem; letter-spacing: 3px; text-transform: uppercase; color: var(--lime); margin-bottom: 16px; font-weight: 500; }
        .section-title { font-family: var(--font-display); font-size: clamp(2rem, 4vw, 3.5rem); font-weight: 800; letter-spacing: -2px; line-height: 1.05; max-width: 600px; }

        /* FEATURES */
        .features-section { padding: 100px 24px; }
        .features-inner { max-width: 1100px; margin: 0 auto; }
        .features-tabs { display: flex; gap: 8px; margin-top: 48px; border-bottom: 1px solid var(--border); padding-bottom: 0; }
        .tab-btn { background: none; border: none; padding: 12px 24px; font-family: var(--font-body); font-size: 0.9rem; color: var(--muted); cursor: pointer; border-bottom: 2px solid transparent; margin-bottom: -1px; transition: all 0.2s; }
        .tab-btn.active { color: var(--lime); border-bottom-color: var(--lime); }
        .tab-btn:hover:not(.active) { color: var(--text); }
        .tab-content { display: grid; grid-template-columns: 1fr 1fr; gap: 64px; align-items: center; margin-top: 56px; }
        .tab-text h3 { font-family: var(--font-display); font-size: 2rem; font-weight: 800; letter-spacing: -1px; margin-bottom: 16px; }
        ...
        .perk { display: flex; align-items: center; gap: 12px; font-size: 0.9rem; color: var(--muted); }
        .perk-icon { width: 28px; height: 28px; background: rgba(200,241,53,0.1); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; flex-shrink: 0; }
        .tab-visual { background: var(--surface); border: 1px solid var(--border); border-radius: 20px; padding: 32px; min-height: 280px; position: relative; overflow: hidden; }
        .tab-visual::before { content: ''; position: absolute; top: 0; right: 0; width: 200px; height: 200px; background: radial-gradient(ellipse at top right, rgba(200,241,53,0.06) 0%, transparent 70%); }
        .visual-doc { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; padding: 20px; }
        .visual-doc-title { font-size: 0.75rem; color: var(--muted); margin-bottom: 12px; display: flex; align-items: center; gap: 8px; }
        .visual-lines { display: flex; flex-direction: column; gap: 8px; }
        .visual-line { height: 8px; border-radius: 4px; background: var(--border); }
        ...
        .flashcard { background: var(--surface2); border: 1px solid rgba(200,241,53,0.2); border-radius: 12px; padding: 24px; text-align: center; }
        .flashcard-q { font-size: 0.75rem; color: var(--lime); margin-bottom: 12px; }
        .flashcard-text { font-size: 1rem; font-weight: 500; margin-bottom: 8px; }
        .flashcard-btns { display: flex; gap: 8px; justify-content: center; }
        .fc-btn { padding: 8px 20px; border-radius: 100px; font-size: 0.8rem; cursor: pointer; border: none; font-family: var(--font-body); }
        .fc-btn.wrong { background: rgba(255,80,80,0.15); color: #ff5050; }
        .fc-btn.right { background: rgba(200,241,53,0.15); color: var(--lime); }
        .quiz-item { background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; padding: 20px; }
        .quiz-q { font-size: 0.9rem; font-weight: 500; margin-bottom: 16px; }
        .quiz-options { display: flex; flex-direction: column; gap: 8px; }
        .quiz-opt { padding: 10px 16px; border-radius: 8px; border: 1px solid var(--border); font-size: 0.85rem; color: var(--muted); transition: all 0.2s; }
        ...
        .pricing-card { background: var(--surface); border: 1px solid var(--border); border-radius: 24px; padding: 40px; position: relative; transition: transform 0.2s; }
        .pricing-card:hover { transform: translateY(-4px); }
        .pricing-card.featured { border-color: var(--lime); background: linear-gradient(135deg, rgba(200,241,53,0.04) 0%, var(--surface) 100%); }
        .pricing-badge-top { position: absolute; top: -12px; left: 32px; background: var(--lime); color: #000; font-size: 0.75rem; font-weight: 500; padding: 4px 14px; border-radius: 100px; }
        .pricing-plan { font-size: 0.8rem; letter-spacing: 2px; text-transform: uppercase; color: var(--muted); margin-bottom: 16px; }
        .pricing-price { font-family: var(--font-display); font-size: 3.5rem; font-weight: 800; letter-spacing: -2px; line-height: 1; }
        ...
        .pricing-feature { display: flex; align-items: center; gap: 10px; font-size: 0.9rem; }
        ...
        .btn-pricing { width: 100%; padding: 14px; border-radius: 100px; font-family: var(--font-body); font-size: 0.95rem; font-weight: 500; cursor: pointer; border: none; transition: all 0.2s; text-decoration: none; display: block; text-align: center; }
        ...
        .btn-pricing.solid { background: var(--lime); color: #000; }
        .btn-pricing.solid:hover { background: var(--lime-dark); }

        /* CTA */
        .cta-section { padding: 100px 24px; text-align: center; }
        .cta-inner { max-width: 700px; margin: 0 auto; background: var(--surface); border: 1px solid var(--border); border-radius: 32px; padding: 72px 48px; position: relative; overflow: hidden; }
        .cta-inner::before { content: ''; position: absolute; bottom: -100px; left: 50%; transform: translateX(-50%); width: 400px; height: 400px; background: radial-gradient(ellipse, rgba(200,241,53,0.08) 0%, transparent 70%); }
        .cta-inner h2 { font-family: var(--font-display); font-size: clamp(2rem, 4vw, 3rem); font-weight: 800; letter-spacing: -2px; margin-bottom: 16px; }
      ...
      <div className="hero">
        ...
        <div className="hero-stats">
          <div className="stat"><div className="stat-num">3x</div><div className="stat-label">più veloce</div></div>
          <div className="stat"><div className="stat-num">5</div><div className="stat-label">doc gratuiti/mese</div></div>
          <div className="stat"><div className="stat-num">100%</div><div className="stat-label">automatico</div></div>
        </div>
      </div>
        ...
        <div className="pricing-grid">
          <div className="pricing-card">
            ...
            <div className="pricing-features">
              ...
              <div className="pricing-feature"><span className="check">✓</span> Quiz a scelta multipla (10 domande)</div>
              <div className="pricing-feature"><span className="check">✓</span> 5-7 slide per il discorso</div>
              ...
              <div className="pricing-feature disabled"><span className="cross">✗</span> Export PowerPoint</div>
            </div>
          ...
          <div className="pricing-card featured">
            ...
            <div className="pricing-features">
              ...
              <div className="pricing-feature"><span className="check">✓</span> 10 slide con 5 punti chiave</div>
              <div className="pricing-feature"><span className="check">✓</span> Export PowerPoint</div>
      ...
      <div className="cta-section">
        <div className="cta-inner">
          ...
          <a href="/signup" className="btn-primary">Inizia gratis — nessuna carta richiesta</a>
        </div>
      </div>
